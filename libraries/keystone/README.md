...
keep-out solid for interference viz), **hole-stamp** (`keystone_cutout()`/
`keystone_boss()` — a consumer `difference()` window and the local positive
material a thin "standard" panel needs to host that window (#38); the
flagship `keystone_insert()` (#54) — a caliper-faithful, printable insert
body), and **fit-check** (`keystone_pitch()`/`keystone_min_pitch()`/
`keystone_pitch_ok()`/`keystone_layout_ok()`/`keystone_pitch_assert()`, a
single-source, style-aware port-spacing guard).

`keystone_insert()` is style-independent (a single measured mechanism, not
keyed to `"standard"`/`"face"`) and **is print-tuned and support-free**
(see its module comment for print orientation); it's consumed as-is by
[keystone-blank](../../projects/keystone-blank), not dropped into
`keystone_cutout()`/`keystone_boss()` — mating it against a panel/slot
cutout is a separate, deferred effort (see Verification below).
...
// The flagship insert (#54) -- caliper-faithful, support-free, printable
// as-is (consumed by projects/keystone-blank). Style-independent: not
// keyed to "standard"/"face", and not yet mated to keystone_cutout()/
// keystone_boss() (see Verification below).
keystone_insert();                     // fit=0.2, latch_wall=1.0, depth=keystone_insert_depth(), blank=true, guides=true
keystone_insert(latch_wall = 1.2);     // PETG-tune the snap-latch beam thickness
```
...
| `keystone_notch(style="standard")` | `[fulcrum_base, fulcrum_protrusion, fulcrum_z, arm_thickness, arm_length, arm_root_z, topnotch_base, topnotch_protrusion, topnotch_z]` (#38) — REAL measured jack-side fulcrum/flex-arm geometry (RESEARCH.md, STL-mesh, `[C]`/`//VERIFY` per field); `"standard"` only. Parked (#54): the flagship `keystone_insert()` derives its own retention geometry from `keystone_insert_lug()`/`keystone_insert_latch()` instead and does not consume this function; reconciling this data to that insert is a deferred, separate effort |
...
| `keystone_insert(fit=0.2, latch_wall=1.0, depth=keystone_insert_depth(), blank=true, guides=true)` | module (#54); the flagship caliper-faithful, support-free printable insert — body + optional guide ribs + fixed retention lug + cantilever snap-fit latch, built from the `keystone_insert_*()` data accessors. `fit`: per-side clearance the body plug is undersized by (slot-engagement clearance; retention features stay anchored to the fitted body regardless of `fit`). `latch_wall`: the ONE print-tunable dimension — the snap-latch beam's wall thickness (PETG-compliance tuning; replaces the caliper's as-measured beam wall verbatim). `depth`: parametric body depth, defaults to `keystone_insert_depth()`; asserts if too shallow for the latch root. `blank`: solid body only (`true`); a functional RJ45 pass-through receptacle (`false`) is deferred/out-of-scope and aborts if requested. `guides`: `true` adds the L/R alignment ribs, `false` omits them. Style-independent (one measured mechanism, not keyed to `"standard"`/`"face"`); mating it against a panel/slot cutout is a separate, deferred effort — see Verification below |

## Verification

The flagship `keystone_insert()` (#54) is verified directly via STL-bbox
geometry checks in `tests/test_keystone_lib.sh`: front-face cross-section
matches the caliper face size less `fit` per side, overall Z-depth matches
`depth`, a latch feature reaches above the body's own top surface, a
retention lug reaches below the body's own bottom surface, `guides=true`
produces more facets than `guides=false`, and the whole insert forms one
connected solid (no disjoint pieces) — render-without-error alone is not
proof of correct geometry. `projects/keystone-blank`
(`tests/test_keystone_blank.sh`, `tests/asserts.scad`) consumes the module
as-is and drift-checks its own dimensions against this library's
accessors (e.g. body Z-extent == `keystone_insert_depth()`).

Mating this insert against a panel/slot cutout is a separate, deferred
effort: `keystone_slot()`/`keystone_notch()` remain the OLD guessed-
mechanism data (parked, not reconciled to this insert), and continue to
back `keystone_cutout()`/`keystone_boss()`'s own `"standard"`/`"face"`
channel geometry and that geometry's own independent verification (real
geometric section checks in `tests/test_keystone_lib.sh` — a slab
...
beyond it). No mate-check between `keystone_insert()` and
`keystone_cutout()`/`keystone_boss()` exists yet.
...
| [Wikipedia, "Keystone module"](https://en.wikipedia.org/wiki/Keystone_module) | B | `keystone_face()` (invariant jack face / plug cross-section); also corroborates `keystone_opening("face")` |
...
- Still `//VERIFY` (flagged for a future research pass, not invented): every
  ...
  *findings* are [C]-corroborated across two independent models each, but
  ...
  all — repo print-process convention, not a keystone-specific spec). Per
...
- All four roles are implemented: data, `keystone_placeholder()`,
  `keystone_cutout()`/`keystone_boss()` (hole-stamp) plus the flagship
  `keystone_insert()` (#54, also hole-stamp), and the fit-check family
  (`keystone_pitch_assert()` included, style-aware). The channel/slit
  geometry (`keystone_cutout()`/`keystone_boss()`, #38) is the real,
  verified `"standard"`/`"face"` mechanism (see Verification above);
  `keystone_insert()` (#54) is the real, caliper-faithful, print-tuned,
  support-free flagship insert (see Verification above) — it does not yet
  mate against `keystone_cutout()`/`keystone_boss()`'s panel-side geometry,
  which remains a separate, deferred effort.
